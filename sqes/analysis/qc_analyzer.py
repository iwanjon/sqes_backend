...
def run_qc_analysis(
    ...
    """
    Main QC Analysis function.
    Fetches all component data for a station, groups them by channel prefix,
    and calculates separate quality scores for each sensor type (e.g., BH, HN).
    """
    station_info = repo.get_station_info(station_code)
    ...
    for sta in station_info:
        ...
        # 1. Fetch ALL QC details for this station/day (pass empty string to get all prefixes)
        dataqc_all = repo.get_qc_details_for_station(tanggal, kode, "")
        
        if not dataqc_all:
            logger.warning(f"<{tipe}> {kode} no QC detail data exist, logging as 'Mati'")
            repo.flush_analysis_result(tanggal, kode, "UK")
            repo.insert_qc_analysis_result(kode, tanggal, '0', 'Mati', tipe, ['Tidak ada data'], "UK")
            continue
        
        # 2. Group the data rows by channel prefix (e.g., 'BH', 'HN')
        prefix_groups = {}
        for row in dataqc_all:
            komp = row[4] if db_type == 'mysql' else row[3]
            prefix = komp[:2] if isinstance(komp, str) and len(komp) >= 2 else "UK"
            
            if prefix not in prefix_groups:
                prefix_groups[prefix] = []
            prefix_groups[prefix].append(row)

        # 3. Run the scoring logic separately for EACH prefix found
        for prefix, dataqc in prefix_groups.items():
            
            # Flush existing final results specifically for this prefix
            try:
                repo.flush_analysis_result(tanggal, kode, prefix)
            except Exception as e:
                logger.error(f"Failed to flush analysis for {kode} ({prefix}): {e}")
                continue
                
            percqc_list = []
            ket = []
            
            # Loop through the components (E, N, Z) for THIS specific prefix
            for qc_row in dataqc:
                try:
                    if db_type == 'mysql':
                        komp = qc_row[4]
            ...
            # Aggregate to station score for THIS prefix
            if not percqc_list:
                score = 0.0
            else:
                if 1.0 in percqc_list:
            ...
            repo.insert_qc_analysis_result(
                ...
                tipe,
                ket,
                channel_prefix=prefix # Pass the current prefix to the database!
            )
            
            logger.info(
                f"{network}.{kode} ({tipe} - {prefix}) QC ANALYSIS FINISH "
                f"(Score: {score:.2f}, Quality: {kualitas}, Components: {len(percqc_list)})"
            )
            time.sleep(0.5)
